<script>
    ...
    import { adminUrl, apiUrl, auth, isLoading } from '../stores';
    ...
    const getApiDatas = async (coins) => {
        const res = await axios.get($apiUrl + '/markets?vs_currency=usd&ids=' + stats.list.join(','))
    ...
    onMount(async () => {
        const coins = await getAdminDatas();
        await getApiDatas(coins)
        // await createList();
        datasLoaded = true;
    });
